...
def save_to_json(func):
    lst = []
    def wrapper(csv_f):
        with open(csv_f, 'r') as src:
            data = csv.DictReader(src)
            for row in data:
                result = func(int(row['a']), int(row['b']), int(row['c']))
                lst.append({'a':int(row['a']), 'b':int(row['b']), 'c':int(row['c']), 'result':result})
            with open('results.json', 'w') as otp:
                json.dump(lst, otp, indent=4)
    return wrapper


def generate_csv_file(file_name, rows):
    data = [{'a':randint(1, 10), 'b':randint(1, 10), 'c':randint(1, 10)} for _ in range(rows)]
    with open(file_name, 'w', newline='') as otp:
        writer = csv.DictWriter(otp, ['a', 'b', 'c'])
        writer.writeheader()
...
if __name__=='__main__':
    ...
    generate_csv_file("input_data.csv", 3)
    find_roots("input_data.csv")
